...
export class OrgUnitFilterComponent implements OnInit, OnDestroy {
  ...
  @Input() orgUnitTreeConfig: any = {
    ...
    level: null,
    loading: true,
    loading_message_selected_org: 'Loading Your Organisation unit...',
    loading_message: 'Loading Organisation units tree...',
    multiple: false,
  ...
  ngOnInit() {
    console.log('orgUnitModel', this.orgUnitModel);
  ...
  emit(e, orgUnitModel) {
    ...
    this.onOrgUnitUpdate.emit({
      ...
      items: this.getSelectedOrgUnitItems(orgUnitModel),
      name: 'ou',
      value: this.getOrgUnitsForAnalytics(orgUnitModel, false),
    });
